...
这个指南根据不同主题，分为多个章节，比如 P 代表编程哲学，I 代表接口设计，
F 表示函数，C 表示类与类层级，R 表示资源管理，Per 表示性能，CP 表示并发与并
行计算，Con 表示常量与不变性等。
...
编译器直接处理代码，程序员也总是直接读代码。所以尽量直接在代码中表达设
计理念。

以下代码中，第一个函数声明的表达内容更清晰： ~month~ 函数返回 ~Month~ 对象，
...
相比较而言，以下第二个函数的声明，表达的代码信息和设计理念就少很多。别
的程序员可能要连蒙带猜地使用你设计的函数，从而，容易产生的 bug。
